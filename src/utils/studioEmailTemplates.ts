import { COMMISSION_STATUSES, PAYMENT_STATUSES, type StudioCommission } from "./studioWorkflow";
...
const renderEditorialEmail = ({
  ...
  const safeAmount = htmlEscape(money(commission.quotedAmount, commission.currency));
...
<body>
  ...
  <article class="shell" role="article" aria-label="Studio update">
    ...
    <section class="body">
      ...
      ${safeCtaLabel && safeCtaUrl ? `<div class="cta-wrap"><a class="cta" href="${safeCtaUrl}">${safeCtaLabel}</a></div>` : ""}
    </section>
    <footer class="footer">
  ...
  if (commission.status === COMMISSION_STATUSES.APPROVED) {
    return {
      ...
      html: renderEditorialEmail({
        ...
        ctaUrl: commission.sessionId ? `https://gxbs.dev/studio/${commission.sessionId}` : undefined,
      }),
    };
  }
...
export const createPaymentCapturedEmail = (commission: StudioCommission) => ({
  ...
  html: renderEditorialEmail({
    ...
    message:
      commission.paymentStatus === PAYMENT_STATUSES.PAID
